...
data.py         Yahoo fetch, Stooq fallback, disk cache, symbol search, demo generator
...
The suggestion field is the one place with no fallback worth the name. Stooq publishes no
symbol search, so when Yahoo's is down the dropdown falls back to a built-in list of common
symbols. Anything outside it has to be typed in full — it still charts, it just stops
suggesting itself.
